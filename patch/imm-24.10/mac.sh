...
start() {
    ...
    # 为 wifi-01 生成随机的后缀部分
    random_suffix_2=$(hexdump -n3 -e '/1 ":%02x"' /dev/urandom)
    # 生成 1 的新 MAC 地址
    NEW_MAC_2="${selected_prefix}${random_suffix_2}"

    # 为 wifi-02 生成随机的后缀部分
    random_suffix_3=$(hexdump -n3 -e '/1 ":%02x"' /dev/urandom)
    # 生成 2 的新 MAC 地址
    NEW_MAC_3="${selected_prefix}${random_suffix_3}"

    # 为 lan1 生成随机的后缀部分
    random_suffix_4=$(hexdump -n3 -e '/1 ":%02x"' /dev/urandom)
    # 生成 1 的新 MAC 地址
    NEW_MAC_4="${selected_prefix}${random_suffix_4}"

    # 为 lan2 生成随机的后缀部分
    random_suffix_5=$(hexdump -n3 -e '/1 ":%02x"' /dev/urandom)
    # 生成 1 的新 MAC 地址
    NEW_MAC_5="${selected_prefix}${random_suffix_5}"

    # 修改 lan的 mac
    uci set network.cfg050f15.macaddr="$NEW_MAC_0"
	  # 修改 wan的 mac
    uci set network.cfg060f15.macaddr="$NEW_MAC_1"

    uci set wireless.default_radio0.macaddr="$NEW_MAC_2"
    uci set wireless.default_radio1.macaddr="$NEW_MAC_3"
    
    ipaddr=$(awk -v a=$RANDOM -v b=$RANDOM -v c=$RANDOM 'BEGIN{srand(); printf "10.%d.%d.%d", a%254+1, b%254+1, c%254+1}')
